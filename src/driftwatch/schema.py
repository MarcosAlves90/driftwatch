...
def typed_definition(key: str, value: dict[str, Any]):
    ...
    if kind == "COLUMN":
        ...
        return ColumnDefinition(**{name: value.get(name) for name in allowed if name in value})  # type: ignore[arg-type]
    if kind == "INDEX":
        ...
        return IndexDefinition(**{name: normalized.get(name) for name in allowed if name in normalized})  # type: ignore[arg-type]
    if kind == "CONSTRAINT":
        ...
        return ConstraintDefinition(**{name: normalized.get(name) for name in allowed if name in normalized})  # type: ignore[arg-type]
