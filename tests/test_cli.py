...
def test_cli_filters_findings_and_renders_compact_text(monkeypatch, tmp_path, capsys):
    config = tmp_path / "config.json"
    config.write_text('{"targets": [{"name": "dev", "connection_string": "fixture"}, {"name": "prod", "connection_string": "fixture"}]}')
    monkeypatch.setattr(cli, "collect", lambda target: Inventory(target.name, {}))
    monkeypatch.setattr(cli, "compare_all", lambda inventories: [
