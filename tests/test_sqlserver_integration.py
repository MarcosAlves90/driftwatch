...
@pytest.mark.skipif(
    not all(CONNECTIONS),
    reason="set DRIFTWATCH_TEST_CONN_A and DRIFTWATCH_TEST_CONN_B for SQL Server integration tests",
)
def test_collects_two_real_sql_server_inventories():
    ...
    assert all(not inventory.errors for inventory in inventories), [
